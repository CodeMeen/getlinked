...
TopNavBar.isExpanded = function () {
  ...
  if (TopNavBar.element.querySelector('.tnb-mobile-accordion-nav').classList.contains('tnb-active')) {
    return true;
  }
...
TopNavBar.closeMenu = function () {
  TopNavBar.element.querySelector('.tnb-mobile-accordion-nav').classList.remove('tnb-active');
...
TopNavBar.openXsMenu = function (x) {
  ...
  if (mobileSection.innerHTML === '') {
    ...
    var xsFilterInputElm = TopNavBar.element.querySelector('#sectionxs_' + x + ' .filter-input');
    xsFilterInputElm.removeAttribute('oninput');
    xsFilterInputElm.addEventListener('input', function (event) {
      TopNavBar.filter(event, 'sectionxs_' + x);
    });
